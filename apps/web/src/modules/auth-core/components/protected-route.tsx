...
export interface ProtectedRouteProps {
  ...
  permission?: string;
  /**
   * Every permission this app checks is a staff permission
   * (`has_staff_permission` in the database), which requires MFA. Defaults
   * to `true` whenever a `permission` is given; pass `false` explicitly for
   * a signed-in-only route that isn't gated by a staff permission.
   */
  requireMfa?: boolean;
}
