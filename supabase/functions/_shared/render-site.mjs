// The static half of the house, as a module with no disk and no environment.
//
// Everything that has been deposited or published is written out as flat HTML
// and plain text, so that a reader can read all of it with JavaScript switched
// off, and so that if the company hosting the database vanished on a Tuesday,
// what had been deposited would still read on Wednesday (Build Specification
// 11.1). It runs in two places from this one file:
//
//   * scripts/prerender.mjs, at build time, writing into dist/ (the fallback
//     copy that ships with every upload);
//   * the render-site Edge Function, whenever staff publish, writing into the
//     public `site` bucket that the Cloudflare Worker serves. That is what
//     lets the CMS publish with no build and no upload.
//
// Both hand this module the same things: the database address and public key,
// the wording register, the SPA shell (index.html), a Bikram Sambat converter,
// and an `emit(path, content)` function that decides where files go.
//
// Deliberately dependency-free (raw fetch against PostgREST with the public
// anon key, same access a browser has).

let SUPABASE_URL;
let SUPABASE_ANON_KEY;
let SITE_URL = "https://paz.com.np";
let API = "";
let FIXTURE = null;
let NepaliDate;
let WORDING = {};
let SINK = () => {
  throw new Error("renderSite was given no emit function.");
};
...
function emit(relPath, content) {
  EMITTED.add(relPath);
  SINK(relPath, content);
}

/** dist-relative file for a public path: "/" is index.html, "/wall" is wall/index.html. */
...
function wHtml(key, htmlVars) {
  ...
  // The NUL markers are deliberate: no wording can contain one.
...
function checkRoutesAndWriteSitemap(requiredRoutes) {
  const problems = [];
  for (const path of SITEMAP.keys()) {
    if (!EMITTED.has(pageFile(path))) problems.push(`sitemap entry ${path} has no page`);
  }
  ...
  for (const path of requiredRoutes ?? []) {
    ...
    if (!EMITTED.has(pageFile(`/${path}`))) problems.push(`public route /${path} was not prerendered`);
  }
...
// ---------------------------------------------------------------------
// RSS 2.0: one feed per publication series, plus one for the Record itself
// (a machine-readable spine, so third parties can mirror the house without
// asking).
// ---------------------------------------------------------------------
const FEED_SERIES = [
...
function rssItem({ title, link: href, description, pubDate, guid }) {
  return [
    "    <item>",
...
function rssFeed({ title, link: href, description, items }) {
  return [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<rss version="2.0">',
    "  <channel>",
...
async function feedRows(table, query) {
  if (FIXTURE) return FIXTURE.tables?.[table] ?? [];
...
async function renderFeeds() {
  for (const series of FEED_SERIES) {
...
export async function renderSite(cfg) {
  SUPABASE_URL = cfg.supabaseUrl;
  SUPABASE_ANON_KEY = cfg.anonKey;
  SITE_URL = (cfg.siteUrl ?? "https://paz.com.np").replace(/\/$/, "");
  ...
  NepaliDate = cfg.NepaliDate;
  WORDING = cfg.wording;
  SINK = cfg.emit;
  baseHtml = cfg.baseHtml;
  if (baseHtml.includes("<main>")) throw new Error("The shell already holds a prerendered page.");

  // A run starts from nothing: nothing is carried over from an earlier one.
  EMITTED.clear();
  SITEMAP.clear();
  PAGES.clear();
  TERMS_DETAILS.clear();
  ...
  await renderFeeds();
  checkRoutesAndWriteSitemap(cfg.requiredRoutes);

  return { written, neCount, sitemapCount: SITEMAP.size, files: EMITTED.size };
}
