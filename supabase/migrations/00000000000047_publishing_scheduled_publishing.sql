-- 0047_publishing_scheduled_publishing.sql
--
...
-- ---------------------------------------------------------------------
-- publishing.transition_item -- create or replace, not a new function:
-- Postgres allows appending new DEFAULT-valued parameters this way
-- without dropping/re-granting (same technique as api.
-- submit_membership_application, 0041). Adds three legal edges:
-- draft/in_review -> scheduled (requires a future scheduled_for),
-- scheduled -> draft (cancel), scheduled -> published (manual "publish
-- now"; the automated job uses a separate function below, not this one,
-- since transition_item requires a signed-in human actor).
-- ---------------------------------------------------------------------
create or replace function publishing.transition_item(
  p_item uuid,
...
-- api.transition_item (0008) is a thin security-invoker wrapper with no
-- p_scheduled_for parameter of its own -- create or replace to pass it
-- through, same append-only-parameter technique as above.
...
-- (0038): no human actor, so item_revisions.created_by and
