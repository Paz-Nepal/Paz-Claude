-- 0048_publishing_transition_notes.sql
--
...
-- ---------------------------------------------------------------------
-- publishing.transition_item -- create or replace, appending p_notes
-- (same technique as p_scheduled_for, 0047).
-- ---------------------------------------------------------------------
...
-- api.transition_item -- create or replace, appending p_notes.
...
-- api.item_revisions -- append notes to the list shape (create or
-- replace function; unlike a view this doesn't need special handling,
-- but the column still has to go at the end of the RETURNS TABLE list).
create or replace function api.item_revisions(p_item uuid)
returns table (
  id uuid,
  revision_no int,
  kind text,
